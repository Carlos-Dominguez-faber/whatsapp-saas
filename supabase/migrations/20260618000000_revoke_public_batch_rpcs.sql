...
-- service_role explicitly. Guarded with to_regprocedure so the migration is a
-- no-op on older schemas where a function does not exist yet.
...
-- Verify after `supabase db push` (all four must be false):
