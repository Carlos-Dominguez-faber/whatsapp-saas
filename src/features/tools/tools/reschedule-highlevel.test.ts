...
const NEW_DATETIME = "2026-06-13T11:00:00-06:00";
...
test("reschedules the active appointment when HighLevel confirms", async () => {
  const { fn, calls } = mockFetch({
    activeAppointment: { id: "appt_local_1", hl_appointment_id: "hl_evt_1" },
    hlStatus: 200,
    hlBody: { id: "hl_evt_1", startTime: NEW_DATETIME },
  });
  const originalFetch = globalThis.fetch;
  globalThis.fetch = fn as typeof fetch;

  try {
    const result = await rescheduleHighLevelTool.run(
    ...
    assert.ok(hlCall, "expected a call to the HighLevel reschedule endpoint");
    assert.equal(hlCall!.method, "PUT");
    assert.equal(hlCall!.headers.Version, "v3");
    assert.deepEqual(hlCall!.body, { startTime: NEW_DATETIME });
...
test("falls back to HighLevel directly when the local appointment row is missing", async () => {
  const { fn, calls } = mockFetch({
    ...
    hlContactAppointments: [
      { id: "hl_evt_9", status: "confirmed", startTime: FUTURE_STARTTIME },
    ],
    hlStatus: 200,
    hlBody: { id: "hl_evt_9", startTime: NEW_DATETIME },
  });
  const originalFetch = globalThis.fetch;
  globalThis.fetch = fn as typeof fetch;
  ...
  try {
    ...
    const hlCall = calls.find((c) => c.url.includes("hl_evt_9"));
    assert.ok(hlCall, "expected the fallback-resolved appointment to be rescheduled");
    ...
    assert.equal(
      updateCall,
      undefined,
      "there was no local row to update",
    );
  } finally {
    globalThis.fetch = originalFetch;
  }
...
test("fails clearly when there is no active appointment locally or in HighLevel", async () => {
  ...
  try {
    ...
    const rescheduleCall = calls.find((c) =>
      c.url.includes("/calendars/events/appointments/"),
    );
    assert.equal(
      rescheduleCall,
      undefined,
      "should not call the reschedule endpoint",
    );
  } finally {
    globalThis.fetch = originalFetch;
  }
...
test("reports the HighLevel error and does not update the local record", async () => {
  const { fn, calls } = mockFetch({
    activeAppointment: { id: "appt_local_1", hl_appointment_id: "hl_evt_1" },
    hlStatus: 400,
    hlBody: { message: "Slot no longer available" },
  });
  const originalFetch = globalThis.fetch;
  globalThis.fetch = fn as typeof fetch;
  ...
  try {
    ...
    assert.equal(
      updateCall,
      undefined,
      "should not update the local appointment when HighLevel rejected it",
    );
  } finally {
    globalThis.fetch = originalFetch;
  }
...
test("skips an appointment row with no hl_appointment_id and finds the HighLevel one", async () => {
  const { fn, calls } = mockFetch({
    ...
    hlStatus: 200,
    hlBody: { id: "hl_evt_2", startTime: NEW_DATETIME },
  });
  const originalFetch = globalThis.fetch;
  globalThis.fetch = fn as typeof fetch;
...
test("filters out past appointments so a stale unclosed booking can't shadow a future one", async () => {
  ...
  try {
    await rescheduleHighLevelTool.run(
      { new_datetime_iso: NEW_DATETIME },
      ctx,
    );
